...
/**
 This class contains the information and represents a commerce event to be
 logged using the mParticle SDK.

 <b>Usage:</b>

 <b>Swift</b>
 <pre><code>
 let commerceEvent = MPCommerceEvent(action: MPCommerceEventAction.AddToCart,
 product: product1)
 ...
 initWithAction:MPCommerceEventActionAddToCart product:product1];
...
/**
 A dictionary containing list names as keys and arrays of products impressed
 under that list name
...
/**
 A promotion container describing a promotion action and its respective
 products.
...
/**
 The label describing the screen on which the commerce event transaction
 occurred
 */
@property(nonatomic, strong, nullable) NSString *screenName;

/**
 The attributes of the transaction, such as: transactionId, tax, affiliation,
 shipping, etc.
...
/**
 A value from the <b>MPCommerceEventAction</b> enum describing the commerce
 event action.
 */
@property(nonatomic) MPCommerceEventAction action;

/**
 The step number, within the chain of commerce event transactions, corresponding
 to the checkout.
 */
@property(nonatomic) NSInteger checkoutStep;

/**
 Flag indicating whether a refund is non-interactive. The default value is
 false/NO.
 */
...
/**
 ...
 @param action A value from the <b>MPCommerceEventAction</b> enum describing the
 commerce event action
 @param product An instance of MPProduct

 @see MPCommerceEventAction
 */
- (nonnull instancetype)initWithAction:(MPCommerceEventAction)action
                               product:(nullable MPProduct *)product;

/**
 Initializes an instance of MPCommerceEvent with a list name for a product
 impression.

 @param listName A string under which the product was listed for this impression
 @param product An instance of MPProduct
 */
- (nonnull instancetype)initWithImpressionName:(nullable NSString *)listName
                                       product:(nullable MPProduct *)product;
...
/**
 ...
 @param promotionContainer An instance of MPPromotionContainer describing a
 promotion action and its respective products

 @see MPPromotionContainer
 */
- (nonnull instancetype)initWithPromotionContainer:
    (nullable MPPromotionContainer *)promotionContainer;
...
/**
 ...
 @param listName A string under which the product was listed for this impression
 */
- (void)addImpression:(nonnull MPProduct *)product
             listName:(nonnull NSString *)listName;
 ...
 */
- (nullable NSArray *)
    allKeys DEPRECATED_MSG_ATTRIBUTE("use customAttributes.allKeys instead");

/**
 A dictionary containing further information about the commerce event. The
 number of entries is limited to 100 key value pairs. Keys must be strings (up
 to 255 characters) and values can be strings (up to 4096 characters), numbers,
 booleans, or dates
 @deprecated use customAttributes instead
 */
- (NSMutableDictionary *_Nullable)
    userDefinedAttributes DEPRECATED_MSG_ATTRIBUTE(
        "use customAttributes instead");
- (void)setUserDefinedAttributes:
    (NSMutableDictionary *_Nullable)userDefinedAttributes
    DEPRECATED_MSG_ATTRIBUTE("set customAttributes instead");
- (nullable id)objectForKeyedSubscript:(nonnull NSString *const)key
    DEPRECATED_MSG_ATTRIBUTE("use customAttributes[key] instead");
- (void)setObject:(nonnull id)obj
    forKeyedSubscript:(nonnull NSString *)key
    DEPRECATED_MSG_ATTRIBUTE("use customAttributes[key] = obj instead");
