...
#define NSStringFromEventType(value)                                           \
  (@{                                                                          \
    ...
    @(MPEventTypeRemoveFromWishlist) :                                         \
        kMPEventTypeStringProductRemoveFromWishlist,                           \
...
typedef NS_ENUM(NSInteger, MPInstallationType) {
  ...
  /** Informs mParticle this binary is the same version. This value is for
     internal use only. It should not be used by developers */
...
/// Survey Providers
typedef NS_ENUM(NSUInteger, MPSurveyProvider) { MPSurveyProviderForesee = 64 };

/// User Identities
/// The identities in this enum are limited to end-user forms of identity. A new
/// enum, MPIdentity, has been provided to cover all valid forms of identity
/// supported by the mParticle Identity API (user identities and device
/// identities)
...
typedef NS_ENUM(NSUInteger, MPIdentity) {
  ...
  /** Device identity advertiser ID (IDFA)
   When setting this, you must also provide the App Tracking Transparency status
   of the device
...
typedef NS_ENUM(NSUInteger, MPILogLevel) {
  ...
  MPILogLevelDebug,
  /** Verbose, debug, warning, and error log messages are displayed on the
     console */
  MPILogLevelVerbose
};
...
typedef NS_ENUM(NSUInteger, MPMessageType) {
  ...
  /** Message type code for when an app successfully registers to receive push
     notifications */
  ...
  /** Message type code for when a user interacts with a received push
     notification */
...
#define NSStringFromMessageType(value)                                         \
  (@{                                                                          \
    ...
    @(MPMessageTypeAppStateTransition) :                                       \
        kMPMessageTypeStringAppStateTransition,                                \
    @(MPMessageTypePushNotification) : kMPMessageTypeStringPushNotification,   \
    @(MPMessageTypeNetworkPerformance) :                                       \
        kMPMessageTypeStringNetworkPerformance,                                \
    @(MPMessageTypeBreadcrumb) : kMPMessageTypeStringBreadcrumb,               \
    @(MPMessageTypeProfile) : kMPMessageTypeStringProfile,                     \
    @(MPMessageTypePushNotificationInteraction) :                              \
        kMPMessageTypeStringPushNotificationInteraction,                       \
    @(MPMessageTypeCommerceEvent) : kMPMessageTypeStringCommerceEvent,         \
    @(MPMessageTypeUserAttributeChange) :                                      \
        kMPMessageTypeStringUserAttributeChange,                               \
    @(MPMessageTypeUserIdentityChange) :                                       \
        kMPMessageTypeStringUserIdentityChange,                                \
...
typedef NS_ENUM(NSUInteger, MPConnectivityErrorCode) {
  ...
  /** The device is not online. Please make sure you've initialized the
     mParticle SDK and that your device has an active network connection. */
  MPConnectivityErrorCodeNoConnection = 1,
  /** Client side error: SSL connection failed to be established due to invalid
     server certificate. mParticle performs SSL pinning - you cannot use a proxy
     to read traffic. */
...
typedef NS_ENUM(NSUInteger, MPIdentityErrorResponseCode) {
  ...
  /** Client side error: There is a current Identity API request in progress.
     Please wait until it has completed and retry your request. */
  MPIdentityErrorResponseCodeRequestInProgress = 1,
  /** Client side error: Request timed-out while attempting to call the server.
     Request should be retried when device connectivity has been reestablished.
   */
  MPIdentityErrorResponseCodeClientSideTimeout = 2,
  /** Client side error: Device has no network connection. Request should be
     retried when device connectivity has been reestablished. */
  ...
  /** HTTP Error 401: Unauthorized. Ensure that you've initialized the mParticle
     SDK with a valid workspace key and secret. */
...
/** Posted immediately after a new session has begun.
 ...
 @discussion You can register to receive this notification using
 NSNotificationCenter. This notification contains a userInfo dictionary, you can
...
/** This constant is used as key for the userInfo dictionary in the
 mParticleSessionDidBeginNotification and mParticleSessionDidEndNotification
 notifications. The value of this key is the id of the session.
 */
extern NSString *_Nonnull const mParticleSessionId;

/** This constant is used as key for the userInfo dictionary in the
 mParticleSessionDidBeginNotification and mParticleSessionDidEndNotification
 notifications. The value of this key is the UUID of the session.
 */
...
/** Posted immediately after the SDK becomes initialized.
 ...
 @discussion You can register to receive this notification using
 NSNotificationCenter. This notification is broadcast when the mParticle SDK
 successfully finishes its initialization.
 */
...
/**
 ...
 for example, specifying a gender of a user. The mParticle platform will look
 for these constants within the user attributes that you have set for a given
 user, and forward any attributes to the services that support them.
...
/** Posted immediately after a kit becomes available to be used.
 ...
 @discussion If your app is calling a kit methods directly, you can register to
 receive this notification when a kit becomes available for use. The
 notification contains a userInfo dictionary where you can extract the
 associated kit instance with the mParticleKitInstanceKey constant.
 @see MPKitInstance
...
/** Posted immediately after a kit becomes unavailable to be used.
 ...
 @discussion If your app is calling kit methods directly, you can register to
 receive this notification when a kit becomes unavailable for use. You may
 receive this notification if a kit gets disabled in the mParticle Services Hub.
 The notification contains a userInfo dictionary where you can extract the
 associated kit instance with the mParticleKitInstanceKey constant.
 @see MPKitInstance
 @see mParticleKitInstanceKey
 */
...
extern NSString
    *_Nonnull const mParticleEmbeddedSDKDidBecomeInactiveNotification;

/**
 Constant used to extract the respective kit instance number from userInfo
 dictionary in a kit notification.
...
/** Posted immediately after the user's MPID changes (or in other terms when a
 * different user becomes active).
 */
extern NSString
    *_Nonnull const mParticleIdentityStateChangeListenerNotification;

/** Key to retrieve now-active user from identity state change notification's
 * userInfo dictionary
 */
extern NSString *_Nonnull const mParticleUserKey;

/** Key to retrieve previously-active user (if applicable) from identity state
 * change notification's userInfo dictionary
 */
