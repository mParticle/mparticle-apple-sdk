...
/**
 ...
 Since this class behaves similarly to an NSMutableDictionary, custom key/value
 pairs can be specified, in addition to the ones listed as class properties.

 <b>For example:</b>

 <b>Swift</b>
 <pre><code>
 let product = MPProduct(name:"Product Name", sku:"s1k2u3", quantity:1,
 price:1.23)
 ...
 <b>Objective-C</b>
 <pre><code>
 MPProduct *product = [[MPProduct alloc] initWithName:&#64;"Product Name"
 sku:&#64;"s1k2u3" quantity:&#64;1 price:&#64;1.23];
...
/**
 The price of a product. If product is free or price is non-applicable use nil.
...
/**
 ...
 @param price The unit price of the product. If the product is free or if
 non-applicable pass 0
...
/**
 ...
 store). If nil, mParticle will use an empty string
 @deprecated use MPTransactionAttributes.affiliation instead
 */
@property(nonatomic, strong, nullable) NSString *affiliation __attribute__((
    deprecated("use MPTransactionAttributes.affiliation instead")));
...
@property(nonatomic, strong, nullable) NSString *currency
    __attribute__((deprecated("use MPCommerceEvent.currency instead")));

/**
 A unique ID representing the transaction. This ID should not collide with other
 transaction IDs. If not specified, mParticle will generate a random id with 20
 characters
 @deprecated use MPTransactionAttributes.transactionId instead
 */
@property(nonatomic, strong, nullable) NSString *transactionId __attribute__((
    deprecated("use MPTransactionAttributes.transactionId instead")));
...
@property(nonatomic, readwrite) double revenueAmount
    __attribute__((unavailable("use MPTransactionAttributes.revenue instead")));

/**
 The total cost of shipping for a transaction. If free or non-applicable use 0.
 Default value is zero
 @deprecated use MPTransactionAttributes.shipping instead
 */
@property(nonatomic, readwrite) double shippingAmount
    __attribute__((deprecated("use MPTransactionAttributes.shipping instead")));

/**
 The total tax for a transaction. If free or non-applicable use 0. Default value
 is zero
 @deprecated use MPTransactionAttributes.tax instead
 */
@property(nonatomic, readwrite) double taxAmount
    __attribute__((deprecated("use MPTransactionAttributes.tax instead")));

/**
 The total value of a transaction, including tax and shipping. If free or
 non-applicable use 0. Default value is zero
 @deprecated use MPTransactionAttributes.revenue instead
 */
@property(nonatomic, readwrite) double totalAmount
    __attribute__((deprecated("use MPTransactionAttributes.revenue instead")));

/**
 The price of a product. If product is free or price is non-applicable use 0.
 Default value is zero
 @deprecated use the price property instead
 */
@property(nonatomic, readwrite) double unitPrice
    __attribute__((deprecated("use the price property instead")));
...
- (nonnull instancetype)initWithName:(nonnull NSString *)name
    ...
    __attribute__((
        unavailable("use initWithName:sku:quantity:price: instead")));
...
- (nonnull instancetype)initWithName:(nonnull NSString *)name
                            category:(nullable NSString *)category
                            quantity:(NSInteger)quantity
                       revenueAmount:(double)revenueAmount
    __attribute__((
        unavailable("use initWithName:sku:quantity:price: instead")));
