...
- (NSURL *)configURL {
    ...
    if ([MParticle sharedInstance].networkOptions.overridesConfigSubdirectory) {
        NSString *configURLFormat = [kMPUrlFormatOverride stringByAppendingString:@"?av=%@&sv=%@"];
...
- (NSURL *)eventURL {
    ...
    if ([MParticle sharedInstance].networkOptions.overridesEventsSubdirectory) {
        NSString *urlString = [NSString stringWithFormat:kMPUrlFormatOverride, kMPURLScheme, eventHost, stateMachine.apiKey, kMPEventsURL];
        _eventURL = [NSURL URLWithString:urlString];
    } else {
        NSString *urlString = [NSString stringWithFormat:kMPUrlFormat, kMPURLScheme, eventHost, kMPEventsVersion, stateMachine.apiKey, kMPEventsURL];
        _eventURL = [NSURL URLWithString:urlString];
    }
...
- (NSURL *)segmentURL {
    ...
    NSString *urlString;
    if ([MParticle sharedInstance].networkOptions.overridesEventsSubdirectory) {
        NSString *segmentURLFormat = [kMPUrlFormatOverride stringByAppendingString:@"?mpID=%@"];
        ...
        NSString *segmentURLFormat = [kMPUrlFormat stringByAppendingString:@"?mpID=%@"];
...
- (NSURL *)identifyURL {
    ...
    if ([MParticle sharedInstance].networkOptions.overridesIdentitySubdirectory) {
        NSString *urlString = [NSString stringWithFormat:kMPIdentityURLFormatOverride, kMPURLScheme, identityHost, pathComponent];
...
- (NSURL *)modifyURL {
    ...
    NSURL *modifyURL;
    if ([MParticle sharedInstance].networkOptions.overridesIdentitySubdirectory) {
        NSString *urlString = [NSString stringWithFormat:kMPModifyURLFormatOverride, kMPURLScheme, identityHost, [MPPersistenceController mpId], pathComponent];
        modifyURL = [NSURL URLWithString:urlString];
    } else {
        NSString *urlString = [NSString stringWithFormat:kMPModifyURLFormat, kMPURLScheme, identityHost, kMPIdentityVersion, [MPPersistenceController mpId], pathComponent];
        modifyURL = [NSURL URLWithString:urlString];
    }
...
- (NSURL *)aliasURL {
    ...
    if (overrides) {
        NSString *urlString = [NSString stringWithFormat:kMPAliasURLFormatOverride, kMPURLScheme, eventHost, stateMachine.apiKey, pathComponent];
        _aliasURL = [NSURL URLWithString:urlString];
    } else {
        NSString *urlString = [NSString stringWithFormat:kMPAliasURLFormat, kMPURLScheme, eventHost, kMPIdentityVersion, kMPIdentityKey, stateMachine.apiKey, pathComponent];
        _aliasURL = [NSURL URLWithString:urlString];
    }
