...
final class MPDataModelTests: XCTestCase {
    ...
    func testTruncateDoesNotApplyNegativeLength() {
        ...
        guard let json = try? JSONSerialization.jsonObject(with: message.messageData, options: []) as? NSDictionary else {
        ...
        guard let truncated = try? JSONSerialization.jsonObject(with: message.messageData, options: []) as? NSDictionary else {
