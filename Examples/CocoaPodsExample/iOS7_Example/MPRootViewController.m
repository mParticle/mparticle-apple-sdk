...
- (void)viewDidLoad {
    ...
    UIImage *mParticleLogo = [UIImage imageNamed:@"mParticle_logo"];
...
- (NSArray *)cellTitles {
    ...
    _cellTitles = @[@"Log Simple Event", @"Log Event", @"Log Screen", @"Log Commerce Event", @"Log Timed Event",
                    @"Log Error", @"Log Exception", @"Set User Attribute", @"Increment User Attribute", @"Set User Identity",
    ...
    selectorNames = @[@"logSimpleEvent", @"logEvent", @"logScreen", @"logCommerceEvent", @"logTimedEvent",
                      @"logError", @"logException", @"setUserAttribute", @"incrementUserAttribute", @"setUserIdentity",
...
- (void)setUserAttribute {
    ...
    [mParticle setUserAttribute:mParticleUserAttributeAge value:age];
    ...
    [mParticle setUserAttribute:mParticleUserAttributeGender value:gender];
    
    // Sets a numeric user attribute using a custom key
    [mParticle setUserAttribute:@"Achieved Level" value:@4];
}

- (void)incrementUserAttribute {
    // Increments the value of a numeric user attribute
    [[MParticle sharedInstance] incrementUserAttribute:@"Achieved Level" byValue:@1];
}

- (void)setUserIdentity {
    MParticle *mParticle = [MParticle sharedInstance];
    
    // Sets identity: customer id
    [mParticle setUserIdentity:@"1357924680" identityType:MPUserIdentityCustomerId];
    
    // Sets identity: email
    [mParticle setUserIdentity:@"john.smith@acme.com" identityType:MPUserIdentityEmail];
}
