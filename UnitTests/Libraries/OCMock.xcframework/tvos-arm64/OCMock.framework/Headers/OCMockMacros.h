...
#define OCMClassMethod(invocation)                                             \
  _OCMSilenceWarnings([[OCMMacroState globalState] switchToClassMethod];       \
                      invocation;);
...
#define OCMVerifyAll(mock)                                                     \
  [(OCMockObject *)mock                                                        \
      verifyAtLocation:OCMMakeLocation(self, __FILE__, __LINE__)]

#define OCMVerifyAllWithDelay(mock, delay)                                     \
  [(OCMockObject *)mock                                                        \
      verifyWithDelay:delay                                                    \
           atLocation:OCMMakeLocation(self, __FILE__, __LINE__)]

#define _OCMVerify(invocation)                                                 \
  ({                                                                           \
    _OCMSilenceWarnings(                                                       \
        [OCMMacroState                                                         \
            beginVerifyMacroAtLocation:OCMMakeLocation(self, __FILE__,         \
                                                       __LINE__)];             \
        @try { invocation; } @catch (...) {                                    \
          [[OCMMacroState globalState] setInvocationDidThrow:YES];             \
          /* NOLINTNEXTLINE(google-objc-avoid-throwing-exception) */           \
...
#define OCMVerify(...)                                                         \
  _OCMVerify_X(, ##__VA_ARGS__, _OCMVerify_2(__VA_ARGS__),                     \
               _OCMVerify_1(__VA_ARGS__))

#define _OCMSilenceWarnings(macro)                                             \
  ({_Pragma("clang diagnostic push")                                           \
        _Pragma("clang diagnostic ignored \"-Wunused-value\"") _Pragma(        \
            "clang diagnostic ignored \"-Wunused-getter-return-value\"")       \
