...
- (instancetype)initWithMessageType:(MPMessageType)messageType session:(MPSession *)session {
    ...
    _messageType = [NSString stringWithCString:MessageTypeName::nameForMessageType(static_cast<MessageType>(messageType)).c_str() encoding:NSUTF8StringEncoding];
...
+ (MPMessageBuilder *)newBuilderWithMessageType:(MPMessageType)messageType session:(MPSession *)session mediaTrack:(MPMediaTrack *)mediaTrack mediaAction:(MPMediaAction)mediaAction {
    ...
    MediaActionDescription actionDescription = MediaControl::actionDescriptionForMediaAction(static_cast<MediaAction>(mediaAction));
