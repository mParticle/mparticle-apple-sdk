...
static NSData *dataValue(sqlite3_stmt *const preparedStatement, const int column);
static NSDictionary *dictionaryRepresentation(sqlite3_stmt *const preparedStatement, const int column);
static double doubleValue(sqlite3_stmt *const preparedStatement, const int column);
static int intValue(sqlite3_stmt *const preparedStatement, const int column);
static int64_t int64Value(sqlite3_stmt *const preparedStatement, const int column);
static NSString *stringValue(sqlite3_stmt *const preparedStatement, const int column);
        ...
        if (sqlite3_prepare_v2(mParticleDB, sqlStatement.c_str(), (int)sqlStatement.size(), &preparedStatement, NULL) == SQLITE_OK) {
            ...
            while (sqlite3_step(preparedStatement) == SQLITE_ROW) {
                ...
                if (userNotification && userNotification.mode == MPUserNotificationModeLocal) {
                    userNotification.userNotificationId = int64Value(preparedStatement, 0);
...
- (void)fetchUserNotificationCampaignHistory:(void (^ _Nonnull)(NSArray<MParticleUserNotification *> * _Nullable userNotificationCampaignHistory))completionHandler {
    dispatch_async(dbQueue, ^{
        ...
        if (sqlite3_prepare_v2(mParticleDB, sqlStatement.c_str(), (int)sqlStatement.size(), &preparedStatement, NULL) == SQLITE_OK) {
            ...
            while (sqlite3_step(preparedStatement) == SQLITE_ROW) {
                NSInteger campaignId = intValue(preparedStatement, 2);
